...
class ConsumerAgent(Agent):
    def __init__(
        self: Agent, id: int, model: Model, beta_params: list, threshold: float
    ) -> None:
        ...
        if model_parameters["social_media_on"]:
            if self.look_to_social_media():
                cons = (
                    ...
                    + model_parameters["social_media_weight"]
                    * self.provider_reputation_in_memory
                )
                self.consumption_probability = max(
    ...
    def compute_consumption_prob_limits(self):
        ...
        self.consumption_probability_limits[0] = max(
            self.trust - deviation_from_trust, 0
        )
        self.consumption_probability_limits[1] = min(
            self.trust + deviation_from_trust, 1
        )
    ...
    def compute_true_utility(self: Agent, item: int) -> float:
        ...
        error = np.random.normal(
            model_parameters["error"]["mu"], model_parameters["error"]["sd"]
        )
    ...
    def update_experience(self: Agent) -> None:
        ...
        if self.is_satisfied():
            self.positive_negative_experience[
                0
            ] += update_consumer_personal_experiences(d)
        else:
            self.positive_negative_experience[
                1
            ] += update_consumer_personal_experiences(d)
    ...
    def consumer_adapt(self: Agent) -> None:
        ...
        if self.consume_item():
            ...
            # save consumed items
            self.model.store_consumed_items(self.consumer_id, self.selecteditem)
